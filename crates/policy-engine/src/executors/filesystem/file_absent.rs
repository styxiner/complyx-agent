...
impl CheckExecutor for FileAbsentExecutor {
    fn check_type(&self) -> &'static str { "file_absent" }
    ...
    async fn execute(
        ...
        match tokio::fs::symlink_metadata(&p.path).await {
            Ok(_) => Ok(EngineCheckResult::fail(
                ...
                "presente",
                "ausente",
                format!("'{}' existe pero no debería", p.path.display()),
            )),
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => Ok(EngineCheckResult::pass(
                check_id,
                "ausente",
                "ausente",
...
mod tests {
    ...
    async fn passes_when_file_absent() {
        let result = FileAbsentExecutor
            .execute("chk-1", &json!({ "path": "/tmp/complyx_test_absent_xyz_999" }))
            .await
            .unwrap();
